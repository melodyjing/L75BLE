/******************************************************************************
	...
	File Name	:	cobra4A_link.c
	Version		:	0.1
	Created	By	:	LiuJian
	Date		:	2020/8/10

	Description :   
       This file contains all the necessary code
  for manipulating the LINKED_LIST data structure.  
	An explination of each function can be found in cobra4A_list.h.
...
/*
	 ...
	 建立一个链表
*/
LINKED_LIST *CreateLinkedList(LINKED_LIST *LinkedListToCreate, int32_t (*CompareTo)(const void *Data1, const void *Data2), void (*CustomFreeMethod)(void *Data))
{
	...
	static void LinkedListFreeNodeAndData(LINKED_LIST *LinkedList, LINKED_LIST_NODE *Node)
	{
		...
		static void LinkedListIteratorRemovalCheck(LINKED_LIST *LinkedList, uint32_t Index)
		{
			if(LinkedList->Iterator.Position > (uint32_t)Index)
			{
				...
				if(LinkedList->Iterator.Position == (uint32_t)Index)
				{
					if(LinkedList->Iterator.CurrentNode->Next == (LINKED_LIST_NODE*)NULL)
					{
						if(LinkedList->Iterator.CurrentNode->Previous == (LINKED_LIST_NODE*)NULL)
						{
							...
							LinkedList->Iterator.Position = (uint32_t)0;
						}
						else
						{
	...
	static LINKED_LIST_NODE *LinkedListNodeRemoval(LINKED_LIST *LinkedList, uint32_t Index)
	{
		LINKED_LIST_NODE *TempNode;
		...
		// Iterator checking
		#if (USING_LINKED_LIST_ITERATOR == 1)
			LinkedListIteratorRemovalCheck((LINKED_LIST*)LinkedList, (uint32_t)Index);
		#endif // end of USING_LINKED_LIST_ITERATOR
	...
	Bool LinkedListDeleteNode(LINKED_LIST *LinkedList, uint32_t Index)
	{
		#if (LINKED_LIST_SAFE_MODE == 1)
			if(LinkedListIsNull(LinkedList))
				return (Bool)FALSE;
			...
			if(LinkedList->Size < (uint32_t)Index)
				return (Bool)FALSE;
		...
		LinkedListFreeNodeAndData((LINKED_LIST*)LinkedList, (LINKED_LIST_NODE*)LinkedListNodeRemoval((LINKED_LIST*)LinkedList, (uint32_t)Index));
	...
	void *LinkedListRemoveNode(LINKED_LIST *LinkedList, uint32_t Index)
	{
		LINKED_LIST_NODE *TempNode;
		void *Data;
		...
		TempNode = (LINKED_LIST_NODE*)LinkedListNodeRemoval((LINKED_LIST*)LinkedList, (uint32_t)Index);
  ...
  void *LinkedListGetData(LINKED_LIST *LinkedList, uint32_t Index)
	{
		uint32_t i;
		LINKED_LIST_NODE *TempNode;
			...
			if(Index > (uint32_t)(LinkedList->Size))
				return (void*)NULL;
		#endif // end of LINKED_LIST_SAFE_MODE

		TempNode = (LINKED_LIST_NODE*)(LinkedList->Head);

		for(i = (uint32_t)1; i < Index; i++)
			TempNode = (LINKED_LIST_NODE*)(TempNode->Next);
	...
	*/
	...
	uint32_t LinkedListContains(LINKED_LIST *LinkedList, const void *Data, Bool MultipleOccurences, uint32_t ReturnOnOccurence)
	{
		LINKED_LIST_NODE *TempNode;
		uint32_t Occurences;
		uint32_t Index;
			...
			if(ReturnOnOccurence > (uint32_t)(LinkedList->Size))
				return (uint32_t)0;
		#endif // end of LINKED_LIST_SAFE_MODE

		TempNode = (LINKED_LIST_NODE*)(LinkedList->Head);
		Occurences = (uint32_t)0;
		Index = (uint32_t)1;

		do
		{
			if(LinkedList->CompareTo((void*)(TempNode->Data), (void*)Data) == (uint32_t)0)
			{
				if(MultipleOccurences || ReturnOnOccurence)
				{
					Occurences++;
...
uint32_t LinkedListGetSize(LINKED_LIST *LinkedList)
{
#if (LINKED_LIST_SAFE_MODE == 1)
		if(LinkedListIsNull(LinkedList))
				return (uint32_t)0;
    ...
    return (uint32_t)(LinkedList->Size);
}
		...
		do
		{
			if(LinkedList->CompareTo((void*)(TempNode->Data), (void*)(Node->Data)) > 0)
			{
				...
				else
				{
					...
					{
						if(Index <= (uint32_t)(LinkedList->Iterator.Position))
							LinkedList->Iterator.Position++;
					}
				#endif // end of USING_LINKED_LIST_ITERATOR
	...
	*/
	...
	Bool LinkedListIteratorHasNext(LINKED_LIST *LinkedList)
	{
		...
		if(LinkedList->Iterator.Position == (uint32_t)0)
			return (Bool)TRUE;
	...
	Bool LinkedListIteratorDelete(LINKED_LIST *LinkedList)
	{
		...
		LinkedListFreeNodeAndData((LINKED_LIST*)LinkedList, (LINKED_LIST_NODE*)LinkedListNodeRemoval((LINKED_LIST*)LinkedList, (uint32_t)(LinkedList->Iterator.Position)));
	...
	void *LinkedListIteratorRemove(LINKED_LIST *LinkedList)
	{
		...
		TempNode = (LINKED_LIST_NODE*)LinkedListNodeRemoval((LINKED_LIST*)LinkedList, (uint32_t)(LinkedList->Iterator.Position));
	...
	void *LinkedListIteratorNext(LINKED_LIST *LinkedList)
	{
		...
		if(LinkedList->Iterator.Position == (uint32_t)0)
		{
			...
			LinkedList->Iterator.Position = (uint32_t)1;
	...
	uint32_t LinkedListIteratorGetIndex(LINKED_LIST *LinkedList)
	{
		#if (LINKED_LIST_SAFE_MODE == 1)
			if(LinkedListIsNull(LinkedList))
				return (uint32_t)0;
		...
		return (uint32_t)(LinkedList->Iterator.Position);
	}
	...
	void *LinkedListIteratorBack(LINKED_LIST *LinkedList)
	{
		...
		if(LinkedList->Iterator.Position == (uint32_t)1 || LinkedList->Iterator.Position == 0)
			return (void*)NULL;
	...
	uint32_t LinkedListGetSizeInBytes(LINKED_LIST *LinkedList, uint32_t DataSizeInBytes)
	{
		uint32_t Size;
		...
		Size = (uint32_t)sizeof(LINKED_LIST);

		if(DataSizeInBytes)
			Size += (uint32_t)(LinkedList->Size * (DataSizeInBytes * sizeof(LINKED_LIST_NODE)));
		else
			Size += (uint32_t)(LinkedList->Size * sizeof(LINKED_LIST_NODE));
	...
	const uint8_t LinkedListLibraryVersion[] = {"Linked List Lib v1.04\0"};

	const uint8_t *LinkedListGetLibraryVersion(void)
	{
		return (const uint8_t*)LinkedListLibraryVersion;
	}
