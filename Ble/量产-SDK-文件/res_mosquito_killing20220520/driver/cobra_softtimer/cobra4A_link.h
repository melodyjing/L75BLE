...
*****************************************************************************/
#ifndef __COBRA4A_LINK_H__
#define __COBRA4A_LINK_H__

#include <stdint.h>
#include <cobra_config.h>
#include "cobra4_common.h"
#include "cobra4A_link_config.h"
...
/*  A pointer to the next node in the LINKED_LIST.  If no next node exists then this will point to NULL. */
	  struct _LinkedListNode *Next;

/*  A pointer to the previous node in the LINKED_LIST.  If no previous node exists then this will point to NULL. */
...
struct _Iterator
{
	...
	*/
	uint32_t Position;					// The numeric position the inside the LINKED_LIST the ITERATOR is at.
...
/* The number of nodes the LINKED_LIST contains.  0 if empty.*/
	 uint32_t Size;					// The current Size of the LINKED_LIST.

/* A pointer to the first node in the LINKED_LIST.  If no nodes exist, then this will point to NULL. */
	 LINKED_LIST_NODE *Head;			// A pointer to the beginning of the LINKED_LIST.

/* A pointer to the last node in the LINKED_LIST.  If no nodes exists, then this will point to NULL. */
...
// Methods
	...
	*/
		...
		int32_t (*CompareTo)(const void *Data1, const void *Data2);
		/*
...
#define LinkedListGetSizeOfNodeInBytes(Size)		(uint32_t)(Size + sizeof(LINKED_LIST_NODE))
#define LinkedListIsEmpty(LinkedList)				(LinkedList->Size == (uint32_t)0)
