...
#define M_TB_KEY_GET_SUBNET_LID(net_key_lid)                    \
        (GETF(net_key_lid, M_TB_KEY_LID) - (M_TB_KEY_DEVICE_LID + 1))
...
/**
 ****************************************************************************************
 * @brief Check if a network key identified by its network key local index is network key
 * used on the primary subnet
 *
 * @return True if the provided network key local index is index of network key for
 * primary subnet, else False
 ****************************************************************************************
 */
bool m_tb_key_net_is_prim(m_lid_t net_key_lid);
 ...
 ****************************************************************************************
 */
void m_tb_key_secure_bcn_rx_ind(m_lid_t net_key_lid, uint8_t flags);
 ...
 ****************************************************************************************
 */
void m_tb_key_get_key_info(uint8_t net_key_lid, uint8_t **p_net_id, uint8_t *p_flags);

/**
 ****************************************************************************************
 * @brief Check if node is part of the primary subnet
 *
 * @return True if node is part of the primary subnet, else False
 ****************************************************************************************
 */
bool m_tb_key_is_on_prim(void);

/**
 ****************************************************************************************
 * @brief Check if an IV Update status can be propagated on a given subnet
 *
 * @param[in] net_key_lid       Network key local index
 *
 * @return True if IV Update status can be propagated, else False
 ****************************************************************************************
 */
bool m_tb_key_can_propagate(m_lid_t net_key_lid);
