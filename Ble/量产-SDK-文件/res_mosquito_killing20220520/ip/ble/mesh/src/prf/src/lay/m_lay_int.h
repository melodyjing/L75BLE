...
enum m_lay_buf_env_flag
{
    ...
    /// (valid only for transmission, used by Friend Layer and Lower Transport Layer)
    M_LAY_BUF_LPN_POS      = 5,
    M_LAY_BUF_LPN_BIT      = 0x20,

    /// Bit[6]: 1 = Message transmitted only to Local Elements (used only by Access Layer)
    /// Meaning never pushed to Upper Transport Layer
    M_LAY_BUF_LOCAL_POS    = 6,
    M_LAY_BUF_LOCAL_BIT    = 0x40,

    /// Bit[7]: 1 = Proxy message, 0 = Normal Message (valid only for transmission and reception)
