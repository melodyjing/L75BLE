...
/// ATT Index                          | ATT UUID                    | Permission        | EXT PERM | MAX ATT SIZE
    ...
    [PLXS_IDX_FEATURES_VAL]          = {ATT_CHAR_PLX_FEATURES_LOC,   RD_P,                0, PLXS_IDX_FEATURES_SIZE_MAX},
...
uint16_t plxs_att2handle(uint8_t attribute)
{
    ...
    uint16_t handle = plxs_env->shdl + attribute;

    if (PLXS_OPTYPE_SPOT_CHECK_ONLY == plxs_env->type_of_operation)
    ...
    if (PLXS_OPTYPE_CONTINUOUS_ONLY == plxs_env->type_of_operation)
...
uint8_t plxs_handle2att(uint16_t handle)
{
    // Get the address of the environment
    struct plxs_env_tag *plxs_env = PRF_ENV_GET(PLXS, plxs);
    uint8_t att_idx = handle - plxs_env->shdl;
 ...
 ****************************************************************************************
 */
static uint8_t plxs_init(struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl, struct plxs_db_cfg* param)
{
    ...
    if (PLXS_OPTYPE_SPOT_CHECK_ONLY == param->type_of_operation)
    { // mask off Continuous Measurement Characteristic
      // mask off Records Access Control Point Characteristic
        cfg_flag[0] = 0x8F;
        cfg_flag[1] = 0x01;
    }
    else if (PLXS_OPTYPE_CONTINUOUS_ONLY == param->type_of_operation)
    { // mask off SPOT-Measurement Characteristic
        cfg_flag[0] = 0xF1;
    }
    else
    {
        param->type_of_operation = 0;
    }
    // Add service in the database
    status = attm_svc_create_db(start_hdl, ATT_SVC_PULSE_OXIMETER, (uint8_t *)&cfg_flag[0],
            PLXS_IDX_NB, NULL, env->task, &plxs_att_db[0],
    ...
    if (status == ATT_ERR_NO_ERROR)
    {
        ...
        plxs_env->type_of_operation = param->type_of_operation;
        /// Keep Supported Features @see common supported_features
        plxs_env->supported_features = param->supported_features;
        /// Keep measurement_status_supported @see common measurement_status_supported
        plxs_env->measurement_status_supported = param->measurement_status_supported;
        /// Keep device_status_supported @see common device_status_supported
        plxs_env->device_status_supported = param->device_status_supported;
        ...
        plxs_env->prf_env.prf_task    = env->task
                |  PERM(PRF_MI, ENABLE);

        // initialize environment variable
        env->id                     = TASK_ID_PLXS;
        plxs_task_init(&(env->desc));

        // Initialize CCC per connection
        memset(plxs_env->prfl_ind_cfg_spot, 0, BLE_CONNECTION_MAX);
        memset(plxs_env->prfl_ind_cfg_cont, 0, BLE_CONNECTION_MAX);
        memset(plxs_env->prfl_ind_cfg_racp, 0, BLE_CONNECTION_MAX);
        memset(plxs_env->racp_in_progress, 0, BLE_CONNECTION_MAX);

        // Store the Features into at database
        {
            uint8_t value[PLXS_IDX_FEATURES_SIZE_MAX];
            uint16_t length;
            uint8_t *ptr = &value[0];
            uint16_t handle = plxs_att2handle(PLXS_IDX_FEATURES_VAL);
            
            co_write16p(ptr, plxs_env->supported_features);
            ptr += 2;
            length = 2;
            
            if (plxs_env->supported_features & PLXP_FEATURE_MEAS_STATUS_PRESENT)
            {
                co_write16p(ptr, plxs_env->measurement_status_supported);
                length += 2; // sizeof(uint16_t)
                ptr += 2;
            }
            if (plxs_env->supported_features & PLXP_FEATURE_DEVICE_SENSOR_STATUS_PRESENT)
            {
                co_write24p(ptr, plxs_env->device_status_supported);
        ...
        for(uint8_t idx = 0; idx < BLE_CONNECTION_MAX ; idx++)
        {
            ...
            ke_state_set(KE_BUILD_ID(env->task, idx), PLXS_FREE);
        }
    }
...
static void plxs_cleanup(struct prf_task_env* env, uint8_t conidx, uint8_t reason)
{
    ...
    plxs_env->prfl_ind_cfg_spot[conidx] = 0;
    plxs_env->prfl_ind_cfg_cont[conidx] = 0;
    plxs_env->prfl_ind_cfg_racp[conidx] = 0;
    plxs_env->racp_in_progress[conidx] = 0;

    
    /* Put PLXS in disabled state */
    ke_state_set(KE_BUILD_ID(env->task, conidx), PLXS_FREE);
}
...
static void plxs_create(struct prf_task_env* env, uint8_t conidx)
{
    ...
    ke_state_set(KE_BUILD_ID(env->task, conidx), PLXS_IDLE);}
