...
#define REG_EM_ADDR_GET(elem, idx) (REG_EM_##elem##_ADDR_GET(idx))
/// Retrieve Exchange index from address load from  HW interface
#define REG_EM_IDX_GET(elem, addr) (((addr) - (EM_##elem##_OFFSET))/(REG_EM_##elem##_SIZE))
#else // (BLE_HW_50_ISO == 1)
/// Retrieve Exchange memory address to set into HW interface
#define REG_EM_ADDR_GET(elem, idx) (REG_EM_##elem##_ADDR_GET(idx) >> 1)
/// Retrieve Exchange index from address load from  HW interface
#define REG_EM_IDX_GET(elem, addr) ((((addr)<<1) - (EM_##elem##_OFFSET))/(REG_EM_##elem##_SIZE))
