...
struct app_ht_env_tag app_ht_env		__attribute__((section("retention_area"),zero_init));
#else
        ...
        ke_task_id_t const src_id)
{
    ...
    if (app_ht_env.temp_value < 0)
        app_ht_env.temp_value = 30;
    if (app_ht_env.temp_value > 250)
        app_ht_env.temp_value = 250;
