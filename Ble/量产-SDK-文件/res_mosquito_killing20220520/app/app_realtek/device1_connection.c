...
LOCAL UINT device1_is_connected ;
	    ...
	    UINT8 * p_data;
			...
			if(connection_devive1_env.receive_begin == 0)
			{
					...
					connection_devive1_env.receive_index = 0;
						...
						{
								 p_data += 1;
								 connection_devive1_env.need_data_number += 1;
						}	
...
void app_device1_send_data_to_uart_server(UCHAR length, UCHAR *p_buffer)
{
  ...
  UINT16 value_len ;
	printf("app_device1_send_data_to_uart_server %x %x\n",connection_devive1_env.conidx,length);
