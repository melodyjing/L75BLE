...
LOCAL UINT        meter_send_channel;
LOCAL  APP_CSEM_PROTOCOL  csem_meter_protocol;
LOCAL METER_WORK_STATE    meter_state;
LOCAL UCHAR       calculate_plus_type;
LOCAL APP_CAL_FREQUENCY   meter_calculate_param;    //目前校准的参数
#if CMD_PD_UPDATE
...
/*
   发射功率加大5DB
*/
LOCAL void app_meter_add5db(void)
{
	  UCHAR temp1;
	  UCHAR temp2;
	  USHORT temp;
...
//秒脉冲
LOCAL void app_key_pin8_function(UCHAR  param)
{
	  ...
	  if(level)
		{
			  ...
			  app_meter_send_event(PLUSE_PER_SECOND, 1);
		}
		else
		{
			  app_m3_gpio_enable_interrupt(GPIO_PIN_8, GPIO_LEVEL_HIGH, app_key_pin8_function);
			  app_meter_send_event(PLUSE_PER_SECOND, 0);
		}	
#if(METER_DEBUG == 1)		
...
//有功脉冲
LOCAL void app_key_pin3_function(UCHAR  param)
{
	    volatile UCHAR level;
...
LOCAL UCHAR meter_communicate_data[250];
LOCAL UCHAR meter_communicate_index = 0x00;
//电表接收数据的处理
...
#else
			if(head == 0xD0)  //ACK DATA
			{
				meter_send_channel = 0xFF;    //设置为最大,表示不再重复发送脉冲数据了
			}
            else if(head == 0xF0)   //通信数据包
            {
                 if(length >= 1)
                 {
                       control_word = cobra_ceva_baseband_csem_register_read(CS565_REG_RXFIFO);
...
//2ms 中断处理
//时序处理
LOCAL void app_iris_meter_calculate_timer_callback(void)
{
#if CMD_PD_UPDATE
	  if((meter_send_channel < 5)&& (meter_state == METER_SEND_DATA)&& (send_pluse_data_ok == 0))

#else
	  if((meter_send_channel < meter_calculate_param.frequency_number)&& (meter_state == METER_SEND_DATA))
#endif
		{
			   app_csem_idle_mode();
...
void app_iris_calculate_meter_task(APP_CAL_FREQUENCY *p_cal_param, UINT address, UCHAR pluse_type)
{
...
#endif
	   memcpy((UCHAR *)&meter_calculate_param, (UCHAR *)p_cal_param, sizeof(APP_CAL_FREQUENCY));
...
/*
   ...
   预留          脉冲类型，                             脉冲电平
                 ...
                 0x06 -时段投切   
*/
void app_meter_send_event(PLUSE_TYPE pluse_type, UCHAR level)
{
	      volatile UCHAR message = 0x00;
	      switch(pluse_type)
				{
					case PLUSE_ACTIVE:
						message |= (0x00 << 1);
...
{	
      ...
      UCHAR *p_addr = iris_dianbiao_get_taiti_address();
