...
LOCAL struct prf_char_def_uuid128 		uart_device1_collector_char[DEVICE1_UART_CHAR_MAX] =
{
    /// Tx Data
	  // 数据发送UUID
 ...
 ****************************************************************************************
 */
LOCAL  int gattc_sdp_svc_ind_handler(ke_msg_id_t CONST msgid,
                                             struct gattc_sdp_svc_ind CONST *ind,
                                             ke_task_id_t CONST dest_id,
                                             ke_task_id_t CONST src_id)
{
    UINT8 state = ke_state_get(dest_id);

    if(state == DEVICE1_UART_DISCOVERING)
    {
        UINT8 conidx = KE_IDX_GET(dest_id);
...
LOCAL int device1_client_uart_enable_req_handler(ke_msg_id_t CONST msgid,
                                   struct device1_client_uart_enable_req CONST *param,
                                   ke_task_id_t CONST dest_id,
                                   ke_task_id_t CONST src_id)
{
	   // Status
    UINT8 status     = PRF_ERR_REQ_DISALLOWED;
 ...
 ****************************************************************************************
 */
LOCAL int gattc_cmp_evt_handler(ke_msg_id_t CONST msgid,
 ...
 ****************************************************************************************
 */
LOCAL int uart_gattc_event_ind_handler(ke_msg_id_t CONST msgid,
                ...
                if(param->handle == device1_uart_env->env[conidx]->bps.chars[DEVICE1_UART_TX_CHAR].val_hdl) 
                {
                    ...
                    struct uart_client_rx_data_ind * ind = KE_MSG_ALLOC(
                            ...
                            uart_client_rx_data_ind);
										...
										memcpy(ind->value, (UINT8*)param->value, ind->length);
                    ke_msg_send(ind);
                }
            } break;
 ...
 ****************************************************************************************
 */
LOCAL int uart_cfg_indntf_req_handler(ke_msg_id_t CONST msgid,
                                       struct uart_client_cfg_indntf_req CONST *param,
                                       ke_task_id_t CONST dest_id,
                                       ke_task_id_t CONST src_id)
{
    UINT16 cfg_hdl = ATT_INVALID_SEARCH_HANDLE;
    // Get the address of the environment
    struct device1_uart_client_env_tag *device1_uart_env = PRF_ENV_GET(DEVICE1_COLLECTOR_UART, device1_uart_client);
    ...
    UINT8 status = PRF_ERR_REQ_DISALLOWED;
	  ...
	  UINT8 msg_status = KE_MSG_CONSUMED;
...
LOCAL int device1_uart_tx_data_to_server_req_handler(ke_msg_id_t CONST msgid,
					       ...
					       //  if ((device1_uart_env->env[conidx]->bps.chars[DEVICE1_UART_RX_CHAR].prop & ATT_CHAR_PROP_WR) == ATT_CHAR_PROP_WR)
						    if ((device1_uart_env->env[conidx]->bps.chars[DEVICE1_UART_RX_CHAR].prop & ATT_CHAR_PROP_WR_NO_RESP) == ATT_CHAR_PROP_WR_NO_RESP)
						    {
								        ...
								        prf_gatt_write(&device1_uart_env->prf_env, conidx, device1_uart_env->env[conidx]->bps.chars[DEVICE1_UART_RX_CHAR].val_hdl,
												    (UINT8 *)param->value, param->length, GATTC_WRITE_NO_RESPONSE);
