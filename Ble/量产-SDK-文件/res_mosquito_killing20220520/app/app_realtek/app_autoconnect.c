...
#if (DIANBIAO_AUTO_ENABLE && DIANBIAO_PROTOCOL_REALTEK_ENABLE)
...
// 寻找断路器是否成功的
void dianbiao_autoconnect_report_find_duanluqi_state(UCHAR error_number, UCHAR * p_mac_address); 
UINT app_iris_autoconnect_get_sample_data(void);
INT app_iris_autoconnect_scan_address(void);
void gudian_uart_module_ask_sample_data_cmd(void);
//判断广播数据是否为特定的
UINT app_gudoan_autoconnect_advdata_paramet_process(UCHAR * buffer,  UCHAR length, USHORT breakerValue);
void iris_dianbiao_autoconnect_report_start_state(UCHAR error_number);
LOCAL UINT32 timer_scan_timeout_index;
LOCAL UINT32 timer_scan_adv_record_index;
LOCAL UINT auto_scan_enable = 0; 
LOCAL UINT duanluqi_number = 0;
LOCAL UINT try_duanluqi_index = 0;
LOCAL struct gap_bdaddr duanluqi_array[MAX_DUANLUQI_NUMBER];  //记录寻找断路器列表, 这个是在蓝牙可以自己解析电流数据的情况下使用
LOCAL ADV_RESULT_INFORMATION adv_result_list[MAX_DUANLUQI_NUMBER];   //这个是在蓝牙不能自己解析电流数据的情况下使用
LOCAL UINT adv_record_number = 0;
LOCAL UINT auto_connect_state = AUTO_CONNECT_STOP_STATE;
LOCAL INT auto_connect_channel;   //  正在尝试连接时用到的 channel
void app_iris_scan_timeout_function(UINT param);
void app_iris_send_modified_character_value(USHORT value);
LOCAL USHORT modified_value;
//等待采样数据的 timer_index
LOCAL UINT32 timer_sample_timeout_index;
LOCAL UINT32 time_sample_number;
USHORT app_auutoconnect_698_produce_head_checksum(UCHAR * p_buffer, INT len);
USHORT app_iris_autoconnect_get_current_character(UINT length, UCHAR *ucDataBuf, \
							USHORT * outDataP);
void app_iris_sample_data_timeout_function(UINT param);
//更改断路器电流参数的返回命令


LOCAL const unsigned char iris_respond_modified_current[] = {
...
//新的更改特征码2的命令返回
LOCAL const unsigned char iris_repond_new_modified_current[] = {
...
void app_iris_scan_timeout_function(UINT param)
{
		...
		{
			 dianbiao_autoconnect_report_find_duanluqi_state(0x04, NULL);  //宣告寻找失败, 失败代码为 0x04
		...
		else
		{
			//开始在稍描到的符合条件的断路器中选择一个开始连接
			 ...
			 {
					dianbiao_autoconnect_report_find_duanluqi_state(0x04, NULL);
					auto_connect_state = AUTO_CONNECT_STOP_STATE;
...
void app_iris_sample_data_timeout_function(UINT param)
{
	  ...
	  //重新发送
		...
		else
		{
			  //报告发送寻找失败
			  // 多次寻找都没有, 也就是多次超时
				dianbiao_autoconnect_report_find_duanluqi_state(0x02, NULL);
				auto_connect_state = AUTO_CONNECT_STOP_STATE;
				cobra_soft_timer_stop(timer_sample_timeout_index);
				cobra_soft_timer_destroy(timer_sample_timeout_index);
...
*/
LOCAL UINT32 timer_ask_delay_index;

void app_iris_ask_delay_timer_function(UINT param)
{
...
LOCAL UINT uiDataBuf[256];

USHORT app_iris_autoconnect_get_current_character(UINT length, unsigned char *ucDataBuf, \
							USHORT * outDataP)
{
    UINT i, j, startPoint, endPoint, levelLength, levelBitNumber, bitNumber,bitNumberH,iStartBit,iHead;
    unsigned char sub,status = 0; 
    bool edge = 0,retValue = FALSE;
    UINT outBuf, parity;
    
    UINT minValue = 0xFFFF, maxValue = 0;
    UINT riseEdgeLimit, dorpEdgeLimit;
...
LOCAL UINT report_adv_data_flag = 0;
...
//void app_iris_scan_timeout_function(UINT param)
LOCAL void app_iris_adv_record_timeout_function(UINT param)
{
...
//串口接收到了启动电流指纹数据采集
void app_iris_autoconnect_process_sample_data(USHORT length, unsigned char *p_buffer)
{
		
		USHORT current_character = 0;  
		USHORT ret;
	...
	//如果不是在寻找断路器时得到这个命令， 那么不做任何处理
	  ...
	  for(i = 0; i < length; i++)
		...
		else
		{
				...
				{
					//发送寻找成功 
						dianbiao_autoconnect_report_find_duanluqi_state(0x00, duanluqi_array[try_duanluqi_index-1].addr.addr);
						auto_connect_state = AUTO_CONNECT_STOP_STATE;
					//设置好断路器连接的地址
						if(auto_connect_channel == CONNECT_CHANNEL1)
						{
...
*/
void app_iris_autoconnect_process(USHORT length, unsigned char * p_buffer)
{
		//USHORT duanluqi_character;
		USHORT ret;
...
//判断广播数据是否为特定的
UINT app_gudoan_autoconnect_advdata_paramet_process(unsigned char * buffer,  unsigned char length, USHORT breakerValue)
{
...
LOCAL const unsigned char new_change_current_cmd[] = 
{	
...
// 寻找断路器是否成功的
void dianbiao_autoconnect_report_find_duanluqi_state(unsigned char error_number, unsigned char * p_mac_address)
{
		USHORT checksum = 0;
...
*/
LOCAL USHORT pppfcs16(USHORT fcs, unsigned char *cp, INT len)
{
		while (len--)
...
USHORT app_auutoconnect_698_produce_head_checksum(unsigned char * p_buffer, INT len)
{
		USHORT fcs;
...
LOCAL const unsigned char iris_modified_current_cmd[] = {
...
UINT app_iris_duanluqi_compare_modified_current_command(unsigned char *p_buffer, unsigned char length)
{
