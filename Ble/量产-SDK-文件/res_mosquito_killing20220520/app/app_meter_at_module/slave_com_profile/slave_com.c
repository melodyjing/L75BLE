...
#if USE_128_UUID
static  const struct attm_desc_128 uart_att_db[UART_IDX_NB] =
{
...
static uint8_t device1_uart_init(struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task,
                            uint8_t sec_lvl, struct device1_uart_db_cfg* params)
{
...
static const struct prf_task_cbs slave_uart_itf =
{
...
{
   return &slave_uart_itf;
}


uint16_t device1_uart_att_hdl_get(struct device1_uart_env_tag* uart_env, uint8_t att_idx)
{
...
uint8_t  device1_uart_att_idx_get(struct device1_uart_env_tag*  uart_env, uint16_t handle)
{
