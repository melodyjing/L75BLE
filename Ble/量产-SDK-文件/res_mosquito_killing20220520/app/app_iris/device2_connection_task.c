/******************************************************************************
	File Name	:	device2_connection_task.c
	Version		:	0.1
	Created	By	:	LiuJian
	Date		:	2019/12/5
...
static int app_connetion_dev2_gapm_activity_created_ind_handler(ke_msg_id_t const msgid,
...
static int app_connetion_dev2_gapm_cmp_evt_handler(ke_msg_id_t const msgid,
...
static int app_connetion_dev2_gapc_connection_req_ind_handler(ke_msg_id_t const msgid,
    ...
    if (connection_devive2_env.conidx != GAP_INVALID_CONIDX)
    {
        ...
        connection_devive2_env.conhdl = param->conhdl;
			...
			//	printf("connection_devive2_env.conhdl dest_id = %d, src_id = %d \n", dest_id, src_id);
			  ...
			  app_device2_exchange_mtu();     //连接后, 设置 交换 MTU 
			
			}
			return (KE_MSG_CONSUMED);
}

static int app_connetion_dev2_gapc_cmp_evt_handler(ke_msg_id_t const msgid,
...
static int app_connetion_dev2_gapc_disconnect_ind_handler(ke_msg_id_t const msgid,
...
static int app_connetion_dev2_mtu_changed_handler(ke_msg_id_t const msgid,
...
static int app_connetion_dev2_gattc_cmp_evt_handler(ke_msg_id_t const msgid,
...
static int device2_connection_gapc_param_update_req_ind_handler(ke_msg_id_t const msgid,
...
static int app_connetion_dev2_gapc_bond_req_ind_handler(ke_msg_id_t const msgid,
                                     ...
                                     ke_task_id_t const src_id)
{
		...
		struct gapc_bond_cfm *cfm = KE_MSG_ALLOC(GAPC_BOND_CFM,
		 ...
		 switch(param->request)
		 {
			 case GAPC_TK_EXCH:
			 {
			  //    uint32_t pin_code = app_device2_get_pincode();  //从广播报文中得到对方的PIN 码
...
static int app_connetion_dev2_gapc_bond_ind_handler(ke_msg_id_t const msgid,
...
static int app_connetion_dev2_gapc_encrypt_req_ind_handler(ke_msg_id_t const msgid,
...
static int app_connetion_dev2_gapc_encrypt_ind_handler(ke_msg_id_t const msgid,
...
static int app_connetion_dev2_gapc_security_ind_handler(ke_msg_id_t const msgid,
