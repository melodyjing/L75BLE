...
{
    /// Tx Data
    [DEVICE3_UART_TX_CHAR]            = {{ 0x79, 0x41, 0xDC, 0x24, 0x0E, 0xE5, 0xA9, 0xE0, 0x93, 0xF3, 0xA3, 0xB5, 0x03, 0x00, 0x40, 0x6E},
                                   ATT_MANDATORY,
                                   ATT_CHAR_PROP_NTF},
    /// Rx data
    [DEVICE3_UART_RX_CHAR]            = {{ 0x79, 0x41, 0xDC, 0x24, 0x0E, 0xE5, 0xA9, 0xE0, 0x93, 0xF3, 0xA3, 0xB5, 0x02, 0x00, 0x40, 0x6E},
                                   ATT_MANDATORY,
                                             ...
                                             ke_task_id_t const src_id)
{
    ...
    if(state == DEVICE3_UART_DISCOVERING)
    {
        ...
        {
		//				printf("device3 gattc_sdp_svc_ind_handler ....................\n");
...
//    if((state == DEVICE3_UART_FREE) && (device3_uart_env->env[conidx] == NULL))
		if((uart_state == DEVICE3_UART_FREE) && (device3_uart_env->env[conidx] == NULL))
    {
        ...
        if(param->con_type == PRF_CON_DISCOVERY)
        {
           ...
           // prf_disc_svc_send(&(device1_uart_env->prf_env), conidx, ATT_SVC_IRIS_UART);
            ...
            device3_uart_env->env[conidx]->last_uuid_req = ATT_SVC_IRIS_UART;
                                ...
                                ke_task_id_t const src_id)
{
    ...
    uint8_t status;
	 ...
	 // printf("device3 gattc_cmp_evt_handler.......\n");
    if (device3_uart_env != NULL)
    {
        uint8_t conidx = KE_IDX_GET(dest_id);
        uint8_t state = ke_state_get(dest_id);

//				printf("device3 gattc_cmp_evt_handler... dest_id = %d, .conidx = %d .state = %d ..\n", dest_id, conidx, state);
        ...
        if(state == DEVICE3_UART_DISCOVERING)
        {
                ...
                (status == ATT_ERR_NO_ERROR))
            {
                ...
                // check characteristic validity
							//	printf(" liujian 111111 ok333333333333333333333333333333333333333 \n");
        ...
        else if(DEVICE3_UART_BUSY)
        {
            switch(param->operation)
            {
                ...
                default: break;
					...
					//	printf("conidx = %d, task id = %d \n", conidx, prf_src_task_get(&device3_uart_env->prf_env, conidx) );
...
void device3_client_set_idle(void)
{
		 struct device3_uart_client_env_tag *device3_uart_env = PRF_ENV_GET(DEVICE3_COLLECTOR_UART, device3_uart_client);
    // Status
			ke_state_set(prf_src_task_get(&device3_uart_env->prf_env, 0), DEVICE3_UART_IDLE);
}
        ...
        switch(param->type)
        {
            ...
            case GATTC_NOTIFY:
            {
                if((param->handle == device3_uart_env->env[conidx]->bps.chars[DEVICE3_UART_TX_CHAR].val_hdl)
                    || ((param->handle == device3_uart_env->env[conidx]->bps.chars[DEVICE3_UART_TX_CHAR].val_hdl)))
                {
...
//		 printf("3333333333333333333 conidx= %d  33333333333333333333333333 \n", conidx);

		 uint8_t uart_state = ke_state_get(((dest_id&0xff) + (conidx << 8)));
	
//    if((device3_uart_env->env[conidx] != NULL) && (ke_state_get(dest_id) == DEVICE3_UART_IDLE))
			
		if((device3_uart_env->env[conidx] != NULL) && (uart_state == DEVICE3_UART_IDLE))
    {
        status = PRF_ERR_INVALID_PARAM;
        //get handle of the configuration characteristic to set and check if value matches property
        switch(param->char_code)
        {
            case DEVICE3_UART_TX_CHAR:            //can only IND
            {
                     ...
                     (param->cfg_val == PRF_CLI_START_NTF))
                {
                    ...
                    cfg_hdl = device3_uart_env->env[conidx]->bps.descs[DEVICE3_UART_DESC_TX_CLI_CFG].desc_hdl;
                }
            }break;
        ...
        if (cfg_hdl != ATT_INVALID_SEARCH_HANDLE)
        {
            ...
            prf_gatt_write_ntf_ind(&device3_uart_env->prf_env, param->connection_index, cfg_hdl, param->cfg_val);
            // Set status
                                       ...
                                       ke_task_id_t const src_id)
{
  ...
  //  uint8_t state = ke_state_get(dest_id);
	...
	//  printf("send device3_uart_tx_data_to_server_req_handler...conidx = %d ...\n", conidx);
    if(state == DEVICE3_UART_IDLE)
    {
	//		  printf("DEVICE3_UART_IDLE \n");
...
//                            (uint8_t *)param->value, param->length, GATTC_WRITE);
									...
									// Send GATT Write Request
                    prf_gatt_write(&device3_uart_env->prf_env, param->connection_index, device3_uart_env->env[conidx]->bps.chars[DEVICE3_UART_RX_CHAR].val_hdl,
                            (uint8_t *)param->value, param->length, GATTC_WRITE_NO_RESPONSE);
									
									
                    // Go to the Busy state
                    ke_state_set(dest_id, DEVICE3_UART_BUSY);
                }
