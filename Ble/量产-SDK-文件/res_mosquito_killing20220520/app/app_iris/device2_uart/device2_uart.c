...
#if USE_128_UUID
static const struct attm_desc_128 device2_uart_att_db[DEVICE2_UART_IDX_NB] =
{


		[DEVICE2_UART_IDX_SVC]                  =   {{0x00, 0x28},  PERM(RD, ENABLE), 0, 0},

		[DEVICE2_UART_IDX_TX_CHAR]        =   { {0x03, 0x28},   PERM(RD, ENABLE), 0, 0},

		[DEVICE2_UART_IDX_TX_VAL]         =   { { 0x79, 0x41, 0xDC, 0x24, 0x0E, 0xE5, 0xA9, 0xE0, 0x93, 0xF3, 0xA3, 0xB5, 0x03, 0x00, 0x40, 0x6E}, PERM(NTF, ENABLE), PERM(UUID_LEN, UUID_128), 0}, //    PERM(NTF, ENABLE), PERM(UUID_LEN, UUID_128), 0},
		
		[DEVICE2_UART_IDX_TX_VAL_IND_CFG]     =   {{0x02, 0x29}, PERM(RD, ENABLE)|PERM(WRITE_REQ, ENABLE), 0, 0},

		
		
	  [DEVICE2_UART_IDX_RX_CHAR]        =   {{0x03, 0x28},   PERM(RD, ENABLE), 0, 0},

		[DEVICE2_UART_IDX_RX_VAL]         =   { { 0x79, 0x41, 0xDC, 0x24, 0x0E, 0xE5, 0xA9, 0xE0, 0x93, 0xF3, 0xA3, 0xB5, 0x02, 0x00, 0x40, 0x6E},  PERM(WRITE_COMMAND, ENABLE)|PERM(WRITE_REQ, ENABLE), PERM(UUID_LEN, UUID_128), MAX_RX_UART}, //PERM(WRITE_COMMAND, ENABLE), 0, MAX_RX_UART } , //PERM(UUID_LEN, UUID_128), MAX_RX_UART},
...
static uint16_t device2_uart_compute_att_table(uint16_t features);
...
static uint8_t device2_uart_init(struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task,
                            uint8_t sec_lvl, struct device2_uart_db_cfg* params)
{
    ...
    cfg_flag = device2_uart_compute_att_table(params->features);
		 ...
		 status = attm_svc_create_db_128(start_hdl, uart_uuid, (uint8_t *)&cfg_flag,
               DEVICE2_UART_IDX_NB, NULL, env->task, &device2_uart_att_db[0], sec_lvl);
    ...
    if( status == ATT_ERR_NO_ERROR )
    {
        ...
        struct device2_uart_env_tag* uart_env =
                (struct device2_uart_env_tag* ) ke_malloc(sizeof(struct device2_uart_env_tag), KE_MEM_ATT_DB);
        ...
        ke_state_set(env->task, DEVICE2_UART_IDLE);
    }
    return (status);
}

static void device2_uart_destroy(struct prf_task_env* env)
{
    struct device2_uart_env_tag* uart_env = (struct device2_uart_env_tag*) env->env;
...
static void device2_uart_create(struct prf_task_env* env, uint8_t conidx)
{
...
static void device2_uart_cleanup(struct prf_task_env* env, uint8_t conidx, uint8_t reason)
{
    /* Clear configuration for this connection */
    struct device2_uart_env_tag* uart_env = (struct device2_uart_env_tag*) env->env;
    uart_env->ntf_ind_cfg[conidx] = 0;
...
static uint16_t device2_uart_compute_att_table(uint16_t features)
{
    ...
    uint16_t att_table = DEVICE2_UART_PROFILE_MASK;
    return att_table;
}
...
static const struct prf_task_cbs device2_uart_itf =
{
    (prf_init_fnct) device2_uart_init,
    device2_uart_destroy,
    device2_uart_create,
    device2_uart_cleanup,
};
   ...
   // struct uart_env_tag* uart_env = PRF_ENV_GET(UART, uart);
    ...
    if(finished)
    {
        ...
        if(uart_env->operation->dest_id != prf_src_task_get(&uart_env->prf_env, 0))
        {
            ...
            struct device2_uart_meas_intv_upd_rsp * rsp =
                    KE_MSG_ALLOC(((uart_env->operation->op == DEVICE2_UART_CFG_MEAS_INTV_IND) ? DEVICE2_UART_MEAS_INTV_UPD_RSP : DEVICE2_UART_TEMP_SEND_RSP),
                                 uart_env->operation->dest_id, prf_src_task_get(&uart_env->prf_env, 0),
                                 device2_uart_meas_intv_upd_rsp);
        ...
        ke_state_set(prf_src_task_get(&(uart_env->prf_env), 0), DEVICE2_UART_IDLE);
    }
}
...
uint8_t device2_uart_update_ntf_ind_cfg(uint8_t conidx, uint8_t cfg, uint16_t valid_val, uint16_t value)
{
    ...
    if(status == GAP_ERR_NO_ERROR)
    {
        ...
        struct device2_uart_cfg_indntf_ind * ind = KE_MSG_ALLOC(DEVICE2_UART_CFG_INDNTF_IND,
                prf_dst_task_get(&uart_env->prf_env, conidx), prf_src_task_get(&uart_env->prf_env, conidx),
                device2_uart_cfg_indntf_ind);
        ind->conidx      = conidx;
...
uint16_t device2_uart_att_hdl_get(struct device2_uart_env_tag* uart_env, uint8_t att_idx)
{
    ...
    do
    {
			 ...
			 if(att_idx >= DEVICE2_UART_IDX_NB)  return ATT_INVALID_HDL;
    ...
    do
    {
        ...
        if(handle < handle_ref)
        {
            break;
				...
				att_idx = (handle - handle_ref) + DEVICE2_UART_IDX_SVC;
				
				if(att_idx >= DEVICE2_UART_IDX_NB) return ATT_INVALID_IDX;
