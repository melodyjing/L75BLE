...
#ifndef __DEVICE2_UART_TASK_H__
#define __DEVICE2_UART_TASK_H__
 ...
 ****************************************************************************************
 */
#define   MAX_UART_SIZE   240
#define   MAX_RX_UART     240 

/// Messages for Health Thermometer Profile Thermometer
/*@TRACE*/
enum  device2_uart_msg_id
{
    /// Start the Health Thermometer Profile Thermometer profile - at connection
    DEVICE2_UART_ENABLE_REQ = TASK_FIRST_MSG(TASK_ID_DEVICE2_UART),//!< HTPT_ENABLE_REQ
    /// Enable confirmation
    DEVICE2_UART_ENABLE_RSP,                               //!< HTPT_ENABLE_RSP

    /// Send temperature value from APP
    DEVICE2_UART_TEMP_SEND_REQ,                            //!< HTPT_TEMP_SEND_REQ
    /// Send temperature response
    DEVICE2_UART_TEMP_SEND_RSP,                            //!< HTPT_TEMP_SEND_RSP

	
    /// Indicate Measurement Interval
    DEVICE2_UART_MEAS_INTV_UPD_REQ,                        //!< HTPT_MEAS_INTV_UPD_REQ
    /// Send Measurement Interval response
    DEVICE2_UART_MEAS_INTV_UPD_RSP,                        //!< HTPT_MEAS_INTV_UPD_RSP
	
		DEVICE2_UART_RX_DATA_IND,
	
    /// Inform APP that Indication Configuration has been changed - use to update bond data
    DEVICE2_UART_CFG_INDNTF_IND,                           //!< HTPT_CFG_INDNTF_IND
};


/// Database Feature Configuration Flags
enum device2_uart_features
{
    /// All Features supported
    DEVICE2_UART_ALL_FEAT_SUP              = 0x1F,
};


/// Notification and indication configuration
enum  device2_uart_ntf_ind_cfg
{
     /// Stable measurement interval indication enabled
    DEVICE2_UART_CFG_STABLE_MEAS_IND    = (1 << 0),
    /// Intermediate measurement notification enabled
    DEVICE2_UART_CFG_INTERM_MEAS_NTF    = (1 << 1),
    /// Measurement interval indication
    DEVICE2_UART_CFG_MEAS_INTV_IND      = (1 << 2),
...
struct device2_uart_enable_req
{
    /// Connection index
    uint8_t conidx;
...
struct device2_uart_enable_rsp
{
    /// Connection index
    uint8_t conidx;
...
struct device2_uart_data_send_req
{
			uint8_t  length;
...
struct device2_uart_meas_intv_upd_req
{
...
struct device2_uart_meas_intv_upd_rsp
{
    /// status
    uint8_t status;
};
...
struct device2_uart_cfg_indntf_ind
{
    /// connection index
    uint8_t  conidx;
