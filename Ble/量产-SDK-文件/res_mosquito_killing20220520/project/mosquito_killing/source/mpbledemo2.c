...
//	  nrf_gpio_pin_set(MPBLEDEMO2_LIGHT);
}
...
{	
	extern uint32_t app_mosquito_killing_statistics_count; 
...
#if (APP_MOSQUITO_KILLING_ENABLE == 1)
	extern uint32_t app_mosquito_killing_statistics_count; 
//	app_wechat_send_data((uint8_t *)&app_mosquito_killing_statistics_count, sizeof(app_mosquito_killing_statistics_count));
	app_mosquito_killing_bt_data_init();
#endif
		m_mpbledemo2_handler->m_data_free_func(data,len);
...
static void mpbledemo2_on_write(void)		
{
	#if 0
	ble_gatts_evt_write_t * p_evt_write = &p_ble_evt->evt.gatts_evt.params.write;
	printf("\r\nble_mpbledemo2_on_ble_evt mpbledemo2_on_write");
	for(uint8_t i=0;i<sizeof(p_evt_write->data[i]);++i){
...
static void mpbledemo2_on_disconnect(void)
{
	mpbledemo2_reset();
}
...
void ble_mpbledemo2_on_ble_evt(void *p_wcs, void *p_ble_evt)
{
	...
	switch (p_ble_evt_t->header.evt_id)
    {
			case BLE_GAP_EVT_CONNECTED:
				#ifdef CATCH_LOG
					printf("\r\nble_mpbledemo2_on_ble_evt BLE_GAP_EVT_CONNECTED");
				#endif
			break;
      case BLE_GAP_EVT_DISCONNECTED:
				#ifdef CATCH_LOG
					printf("\r\nble_mpbledemo2_on_ble_evt BLE_GAP_EVT_DISCONNECTED");
				#endif
				mpbledemo2_on_disconnect(p_wcs_t, p_ble_evt);
			break;
			case BLE_GATTS_EVT_WRITE:
				#ifdef CATCH_LOG
					printf("\r\nble_mpbledemo2_on_ble_evt BLE_GATTS_EVT_WRITE");
				#endif
				mpbledemo2_on_write(p_wcs_t, p_ble_evt);
			break;
			default:
				#ifdef CATCH_LOG
					printf("\r\nble_mpbledemo2_on_ble_evt BLE_GATTS_EVT default");
				#endif
				break;
    }
		#endif
...
void mpbledemo2_data_produce_func(void *args, uint8_t **r_data, uint32_t *r_len)
{
		...
		case CMD_INIT:
			{
				//has challeange
				challeange[0] = cobra_trng_rand_read();
				challeange[1] = cobra_trng_rand_read();
				challeange[2] = cobra_trng_rand_read();
				challeange[3] = cobra_trng_rand_read();
...
int mpbledemo2_data_consume_func(uint8_t *data, uint32_t len)
{
		...
		#ifdef CATCH_LOG
			printf("\r\n magicCode: %02x",bledemohead->m_magicCode[0]);
			printf(" %02x",bledemohead->m_magicCode[1]);
			printf("\r\n version: %d",ntohs(bledemohead->m_version));
			printf("\r\n totalLength: %d",ntohs(bledemohead->m_totalLength));
			printf("\r\n cmdid: %d",ntohs(bledemohead->m_cmdid ));
			printf("\r\n errorCode: %d",ntohs(bledemohead->m_errorCode));
		#endif	
...
void app_wechat_enable_ind(void)
{
	 mpbledemo2Sta.indication_state = true;
}

void app_wechat_disbale_ind(void)
{
	 mpbledemo2Sta.indication_state = false;
}
