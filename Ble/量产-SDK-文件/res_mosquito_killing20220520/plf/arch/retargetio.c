...
#define  HAVE_DEBUG_INFORMATION  1

#if APP_OTABOOTLOAD_ENABLE
#undef   HAVE_DEBUG_INFORMATION
#define  HAVE_DEBUG_INFORMATION  0
#endif
...
//  while (len--)
//  {
//    if ((c = cobra_uart_read(PRINTF_UART)) != -1)
//    {
//      *ptr++ = c;
//      rxCount++;
//    }
...
//  {
//				cobra_uart_write(PRINTF_UART,*ptr++);
//				
//  }
...
static int TxBuf(uint8_t *buffer, int nbytes)
{
  ...
  for (i = 0; i < nbytes; i++)
  {
		rdw_uart_write_byte(PRINTF_UART,*buffer++);
  }
  return nbytes;
}
...
{
		rdw_uart_write_byte(PRINTF_UART,ch);
	
}

int __getchar(void)
{
  return(uartGetChar(PRINTF_UART));
}
...
{
#if HAVE_DEBUG_INFORMATION
#if 0
    if(ch=='\n')
			cobra_uart_write(PRINTF_UART,'\r');

		cobra_uart_write(PRINTF_UART,ch);

	if(ch=='\r')
			cobra_uart_write(PRINTF_UART,'\n');
#else
	if(ch=='\n')
			cobra_uart_write(M3_UART0,'\r');

		cobra_uart_write(M3_UART0,ch);

	if(ch=='\r')
			cobra_uart_write(M3_UART0,'\n');
#endif	
#endif
...
int fgetc(FILE *f)
{
 // return(cobra_uart_read(PRINTF_UART));
	  return cobra_uart_read(M3_UART0);
}
